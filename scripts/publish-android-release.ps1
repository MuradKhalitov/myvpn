...
function Invoke-RemoteScript {
    ...
    # PowerShell runs on Windows while the remote script is executed by Linux bash.
    # Normalize Windows CRLF/CR line endings to Unix LF first.
    #
    # Then send the script as Base64 instead of piping PowerShell text directly
    # to ssh. This prevents CRLF and PowerShell encoding conversion from
    # corrupting the remote bash script.
    $normalizedScript = $Script.Replace("`r`n", "`n").Replace("`r", "`n")
    $scriptBytes = [System.Text.Encoding]::UTF8.GetBytes($normalizedScript)
    $scriptBase64 = [Convert]::ToBase64String($scriptBytes)

    $remoteCommand = "printf '%s' '$scriptBase64' | base64 -d | bash -s -- $Arguments"
...
function Assert-ApkHttpResponse {
    ...
    if ($response.StatusCode -ne 200) {
        throw "APK URL $Uri returned HTTP $($response.StatusCode), expected 200."
    }
...
try {
    if ([string]::IsNullOrWhiteSpace($VersionName)) {
        throw "VersionName must not be empty."
    }
    ...
    if ($VpsHost -notmatch '^[A-Za-z0-9.-]+$') {
        throw "VpsHost has an invalid format."
    }

    if ($VpsUser -notmatch '^[A-Za-z_][A-Za-z0-9_-]*$') {
        throw "VpsUser has an invalid format."
    }
    ...
    foreach ($path in @(
        ...
        if (-not (Test-Path -LiteralPath $path)) {
            throw "Required Android release path is missing: $path"
        }
    }

    $signingProperties = Read-JavaProperties $signingPropertiesFile

    $storeFile = Get-RequiredProperty `
        $signingProperties `
        "storeFile"

    Get-RequiredProperty `
        $signingProperties `
        "storePassword" | Out-Null
    ...
    $currentVersionCodeText =
    $versionCodeMatches[0].Groups["code"].Value

    if (-not [int]::TryParse(
            $currentVersionCodeText,
            [ref]$currentVersionCode
    ) -or
            $currentVersionCode -le 0) {
        throw "The existing versionCode must be a positive integer."
    }

    $currentVersionName =
    $versionNameMatches[0].Groups["name"].Value
    ...
    $versionedApkUrl =
    "$PublicBaseUrl/downloads/$apkName"

    $remote =
    "$VpsUser@$VpsHost"

    $recoveryCommand =
    ".\scripts\publish-android-release.ps1 -VersionName `"$VersionName`" -VpsHost `"$VpsHost`" -VpsUser `"$VpsUser`""
    ...
    if ($DryRun) {
        ...
        if ($isResume) {
            Write-Output "Mode: resume existing version without another versionCode increment."
        }

        Write-Stage 1 "Version update"

        Write-Output (
        "Would persist versionCode = {0} and versionName = {1} in android/app/build.gradle.kts." `
                -f $newVersionCode, $VersionName
        )
        ...
        Write-Output (
        "ssh {0}: cd {1}; test APK; ln -sfn {2} myvpn-latest.apk; readlink -f myvpn-latest.apk" `
                -f `
                $remote,
        $RemoteDownloadsDirectory,
        $apkName
        ...
        Write-Output (
        "ssh {0}: cd {1}; atomically update only ANDROID_LATEST_VERSION_CODE={2}, ANDROID_LATEST_VERSION_NAME={3}, ANDROID_APK_URL={4} in .env.staging" `
                -f `
                $remote,
        ...
        Write-Output (
        "ssh {0}: readlink -f {1}/myvpn-latest.apk" `
                -f `
                $remote,
        $RemoteDownloadsDirectory
        )
    ...
    $persistedGradleContent =
    Get-Content `
            -LiteralPath $gradleFile `
            -Raw

    $persistedVersionCodeMatches =
    [regex]::Matches(
            $persistedGradleContent,
            $versionCodePattern
    )

    $persistedVersionNameMatches =
    [regex]::Matches(
            $persistedGradleContent,
            $versionNamePattern
    )

    if ($persistedVersionCodeMatches.Count -ne 1 -or
            $persistedVersionNameMatches.Count -ne 1) {
        throw "Version update was not persisted in the expected Gradle structure."
    }

    [int]$persistedVersionCode = 0

    $persistedVersionCodeText =
    $persistedVersionCodeMatches[0].Groups["code"].Value

    $persistedVersionName =
    $persistedVersionNameMatches[0].Groups["name"].Value

    $persistedCodeIsExpected =
    [int]::TryParse(
            $persistedVersionCodeText,
            [ref]$persistedVersionCode
    ) -and
            $persistedVersionCode -eq $newVersionCode

    if (-not $persistedCodeIsExpected -or
            $persistedVersionName -ne $VersionName) {
    ...
    Test-Path `
            ...
            (Get-Item -LiteralPath $sourceApk).Length -le 0) {
    ...
    & scp `
        $releaseApk `
        "${remote}:$RemoteDownloadsDirectory/$apkName"
    ...
    Invoke-Remote `
        $remote `
        "cd '$RemoteDownloadsDirectory' && test -s '$apkName' && ln -sfn '$apkName' myvpn-latest.apk" `
        "Latest symlink update"

    Invoke-Remote `
        $remote `
        "test `"`$(readlink -f '$RemoteDownloadsDirectory/myvpn-latest.apk')`" = '$RemoteDownloadsDirectory/$apkName'" `
    ...
    $safeRemoteArguments =
    "'$newVersionCode' '$VersionName' '$LatestApkUrl'"
...
awk -v code="$version_code" -v name="$version_name" -v url="$apk_url" '
  /^ANDROID_LATEST_VERSION_CODE=/ {
    print "ANDROID_LATEST_VERSION_CODE=" code
    seen_code=1
    next
  }

  /^ANDROID_LATEST_VERSION_NAME=/ {
    print "ANDROID_LATEST_VERSION_NAME=" name
    seen_name=1
    next
  }

  /^ANDROID_APK_URL=/ {
    print "ANDROID_APK_URL=" url
    seen_url=1
    next
  }
...
grep -Fqx \
    "ANDROID_LATEST_VERSION_CODE=$version_code" \
    "$env_file"

grep -Fqx \
    "ANDROID_LATEST_VERSION_NAME=$version_name" \
    "$env_file"
...
'@
    ...
    Invoke-Remote `
        $remote `
        "test `"`$(readlink -f '$RemoteDownloadsDirectory/myvpn-latest.apk')`" = '$RemoteDownloadsDirectory/$apkName'" `
        "Final latest symlink validation"
    ...
    $metadata =
    Invoke-CurlJson `
            "$PublicBaseUrl/api/v1/app/version" `
            "App-version metadata request"

    if ($metadata.latestVersionCode -ne $newVersionCode -or
            $metadata.latestVersionName -ne $VersionName -or
            $metadata.apkUrl -ne $LatestApkUrl) {
    ...
    [Console]::Error.WriteLine(
            "Release failed at ${currentStage}: $($_.Exception.Message)"
    )

    if ($versionPersisted) {
        [Console]::Error.WriteLine(
                "The local Gradle version remains versionCode=$newVersionCode, versionName=$VersionName."
        )

        [Console]::Error.WriteLine(
                "Recovery command (the same VersionName reuses this versionCode):"
        )
    ...
    if ($remoteApkUploaded) {
        [Console]::Error.WriteLine(
                "Partial release: the versioned APK was uploaded to $RemoteDownloadsDirectory/$apkName."
        )
    }

    if ($latestSymlinkSwitched) {
        [Console]::Error.WriteLine(
                "Partial release: myvpn-latest.apk already points to $apkName; backend metadata may still require recovery."
        )
    }
